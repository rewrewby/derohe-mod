...
func CheckIfMiniBlockIsOrphaned(local bool, mblData block.MiniBlock, miner string) {
	...
	for {
		...
		if chain.Get_Height() >= int64(mblData.Height)+1 {
			...
			if local {
				globals.Console_Only_Logger.Info(fmt.Sprintf(red+"Height: %d"+reset_color+" - "+red+"%s"+reset_color+": "+blue+"Orphan DERO mini block\t"+yellow+"("+red+"Profit Loss 💣"+yellow+")"+reset_color+reset_color, mblData.Height, miner))
			} else {
				if config.RunningConfig.TraceBlocks {
					globals.Console_Only_Logger.Info(fmt.Sprintf(red+"Height: %d"+reset_color+" - "+red+"%s"+reset_color+": "+blue+"Orphan DERO mini block"+reset_color, mblData.Height, miner))
				}
			}
